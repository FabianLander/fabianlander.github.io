...
date: 2025-02-28
description: "Seminar notes exploring Vorobets' approach to the Veech alternative — the dichotomy in geodesic flow behavior on translation surfaces: trajectories are either periodic or uniformly distributed."
tags:
...
> **Note:** These notes reflect my understanding of our discussions – if it were a game of telephone, consider this the final whisper. All errors and clunky phrasings are surely mine; all good ideas belong to the speakers and authors of the reference material.
...
Given an interval $I = [a,b] \subset \mathbb{R}$ and points $S= \{x_1, \dots, x_m \}$ where $a < x_1 < x_2 < \dots < x_m < b$, an *IET* is a map $T: I\setminus S \rightarrow I$ that acts as a translation (shift) on each of the components of $I\setminus S$.
...
<div class="proof">
Both parts of this formula do not change when (removable) singular points are added or deleted, so we may assume that $\omega$ has (potentially removable) singular points.
...
Let's explore the second consequence in more detail. Let $v_1, v_2$ be linearly independent vectors in $\mathbb{R}^2$. By $\mathbb{T}_{v_1, v_2}$ we denote the quotient space of $\mathbb{R}^2$ by the subgroup $\mathbb{Z}v_1 \oplus \mathbb{Z}v_2$. Then $\mathbb{T}_{v_1, v_2}$ is a torus, the canonical projection $\pi: \mathbb{R}^2 \to \mathbb{T}_{v_1, v_2}$ is a local homeomorphism, and the continuous maps from domains in $\mathbb{T}_{v_1, v_2}$ into $\mathbb{R}^2$ that are right inverse to $\pi$ define on $\mathbb{T}_{v_1, v_2}$ a translation structure without singular points. This structure is called a *flat torus*.
...
Let $l_2$ be the distance from $x''$ to $x$ when moving along the direction $e_1$. Then the trajectories parallel to $v_2 = w_1e_2 + l_2e_1$ form a pencil of periodic trajectories of length $|v_2|$. The trajectories from the pencils parallel to $e_1$ and $v_2$ intersect one another just once. This implies that the translation structure $\omega$ is isomorphic to the planar torus $\mathbb{T}_{l_1e_1, v_2}$.
</div>

It is well known that on $\mathbb{T}_{v_1, v_2}$, flows parallel to vectors in $\mathbb{Z}v_1 \oplus \mathbb{Z}v_2$ are periodic, while flows in all other directions are strongly ergodic. This dichotomy represents the simplest possible dynamical behavior for geodesic flows on translation surfaces.
...
Let $\omega$ be a translation surface. Given an element $a\in \mathrm{SL}(2,\mathbb{R})$ we define $a\omega$ as the translation surface obtained by post-composing charts $(U_\alpha, f_\alpha)$ of $\omega$ with $a$, i.e. $(U_\alpha, a\circ f_\alpha)$. This defines an action on the space of translation structures and we denote by $\Gamma(\omega)$ the stabilizer subgroup of $\omega$, i.e.,

$$
\Gamma(\omega) = \{a\in \mathrm{SL}(2,\mathbb{R}) \, \vert \, a\omega \text{ is isomorphic to } \omega\}.
$$
...
We denote the minimal length of saddle connections in a translation structure $\omega$ by $m(\omega)$. For fixed $\omega$, we define the map

$$
\begin{aligned}
d: \mathrm{GL}(2,\mathbb{R}) &\longrightarrow \mathbb{R} \\
...
The map $d$ is continuous and bounded on $\mathrm{SL}(2,\mathbb{R})$.
<div class="thm-content">
</div>
</div>

<div class="proof">
Let $v\in\mathbb{R}^2$ be a development of the shortest saddle connection of $a\omega$, so $|v| = d(a)$. Then for $b\in \mathrm{GL}(2,\mathbb{R})$ we have

$$
(ba^{-1})v \in SC(b\omega)
$$

and

$$
d(b) \le |ba^{-1} v| \le \Vert ba^{-1} \Vert \cdot d(a),
$$

where $\Vert \cdot \Vert$ denotes the operator norm on $\mathrm{GL}(2,\mathbb{R})$. Exchanging the roles of $a$ and $b$, we get

$$
\Vert ab^{-1} \Vert^{-1} \cdot d(a) \le d(b) \le \Vert ba^{-1} \Vert \cdot d(a).
$$

So if $a\rightarrow b$ then $\Vert ab^{-1}  \Vert \rightarrow 1$ and $d(b) \rightarrow d(a)$ which proves the continuity of $d$. Acting on $\omega$ by an element of $\mathrm{SL}(2,\mathbb{R})$ doesn't change the area $S$ of $\omega$. To show that $d$ is bounded on $\mathrm{SL}(2,\mathbb{R})$, we will prove that $m(a\omega) \le \sqrt{2\text{Area}(a\omega)} = \sqrt{2\text{Area}(\omega)}$. For this, take a segment $I$ of length $\sqrt{S}$ in an arbitrary direction, and starting in some singular point $A$. We assume that this segment doesn't contain a saddle point besides $A$. Drawing another segment of length $\sqrt{S}$ orthogonal to $I$, also with no other singularities, gives rise to a rectangle of area $S$. We are going to assume that this rectangle contains no singularity since otherwise we would have a saddle connection of length at most $\sqrt{S}$. Now consider a maximal segment $J$ in $I$ on which the first return time of the flow orthogonal to $I$ is constant. This return time is at most $\sqrt{S}$. The boundary of $J$ maps back to $I$.
...
$\Gamma(\omega)$ is a non-uniform discrete subgroup of $\mathrm{SL}(2,\mathbb{R})$.
<div class="thm-content">
</div>
</div>

By non-uniform we mean that the quotient $\mathrm{SL}(2,\mathbb{R})/\Gamma(\omega)$ is non-compact.
...
To show that $\Gamma(\omega)\backslash\mathrm{SL}(2,\mathbb{R})$ is non-compact, let $v\in SC(\omega)$ and $a\in\mathrm{SL}(2,\mathbb{R})$ such that $|av| \le \frac{1}{2}|v|$. Then
...
If $\mathrm{SL}(2,\mathbb{R})/\Gamma(\omega)$ were compact, we could find a sequence $\gamma_n \in \Gamma(\omega)$ such that $a^n\gamma_n \rightarrow b$.
</div>
...
<div class="thm-content">
If $\Gamma(\omega)$ is a lattice in $\mathrm{SL}(2,\mathbb{R})$, then $\omega$ is elementary. Furthermore, the following are equivalent:
...
H. Masur, "Hausdorff dimension of the set of nonergodic foliations of a quadratic differential", <em>Duke Math. J.</em> 66 (1992), 387-442.
</div>
...
$$
g^t = \begin{pmatrix} e^{t/2} & 0 \\ 0 & e^{-t/2} \end{pmatrix} \in \mathrm{SL}(2, \mathbb{R}).
$$
...
Let $T$ be an IET on $I = [a,b]$ and $a_0 = a, a_1, \dots, a_k = b$ be the break points of $T$. Denote by $\varepsilon (T)$ the length of the shortest interval(s) of this partition. For each $n\in \mathbb{N}$ we write $\varepsilon_n(T) = \varepsilon(T^n)$.
...
transformation", <em>Ergodic Theory Dynamical Systems</em> 7 (1987), 149-153.
</div>
and
<div style="font-size: 0.9em; margin-top: 1em; margin-bottom: 1em; padding-left: 1.5em;">
